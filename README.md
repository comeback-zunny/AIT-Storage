...
- `PLAYING`, `HISTORY`, `앱 아이콘`은 먼저 게임 목록을 보여주고, 게임을 선택하면 하위 폴더별 미리보기를 표시합니다.
- 게임 화면에서는 하위 폴더별로 최대 5장만 보여주고, `전체 보기`로 해당 폴더의 모든 이미지를 확인할 수 있습니다.
- `playing/thumbnail.png`, `history/thumbnail.png`, `app-icon/thumbnail.png`는 첫 화면 버튼 배경으로 사용합니다.
- `playing/게임명/thumbnail.png`, `history/게임명/thumbnail.png`는 게임 카드 배경으로 사용합니다.
- `app-icon/게임명/app-icon.png`는 앱 아이콘 게임 카드 배경으로 사용합니다.
- 썸네일을 클릭하면 원본 이미지를 모달로 확인하고 이전/다음 이미지로 이동할 수 있습니다.
- 동영상은 목록 카드에서 바로 재생할 수 있는 플레이어로 표시합니다.
